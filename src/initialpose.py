...
def findInitialTeeth(radiograph):
    ...
    pos = [np.array((rho[0]-35+sideSize,110+upperJawLine-upperJawSize))]
    for i in range(1,3):
        pos.append(np.array((rho[i-1]+(rho[i]-rho[i-1])/2+sideSize,130+upperJawLine-upperJawSize)))
    pos.append(np.array((rho[2]+25+sideSize,110+upperJawLine-upperJawSize)))
    rho,theta = zip(*lowerLines)
    pos.append(np.array((rho[0]-40+sideSize,50+lowerJawLine)))
    for i in range(1,3):
        pos.append(np.array((rho[i-1]+(rho[i]-rho[i-1])/2+sideSize,50+lowerJawLine)))
    pos.append(np.array((rho[2]+40+sideSize,50+lowerJawLine)))
